...
object DocumentationGenerator extends App {
  ...
  val communityContents = Seq(
    ...
    CommunityContent(
      ...
      "Xebia blog (jbargsten)"
    ),
    CommunityContent(
      "Lagom 1.4 and Kubernetes Orchestration",
    ...
    CommunityContent(
      "Tour de Lagom (blog post series)",
    ...
    CommunityContent(
      "The Lagom Framework for Microservices and Domain-Driven Design",
...
//    VideoLink(description, href),
    VideoLink(
      ...
      "https://youtube.com/watch?v=Sx8ctqLxDf0"
    ),
    VideoLink(
      "EventSourcing and CQRS with Lagom Microservice Framework",
      "https://youtube.com/watch?v=BuyXxw79fKk"
    ),
    VideoLink(
      "CQRS and Event Sourcing with Lagom by Miel Donkers",
      "https://youtube.com/watch?v=1VOfcuWOuy8"
    ),
    VideoLink(
      "Event Sourcing and CQRS by Lutz Huehnken",
      "https://youtube.com/watch?v=Z6_Nd7lu2PI"
    ),
    VideoLink(
      "Events-First Microservices with Lagom by Gideon de Kok",
      "https://youtube.com/watch?v=f1YSSaI_J-c"
    ),
    VideoLink(
      "Lagom in Practice by Yannick De Turck",
      "https://youtube.com/watch?v=JOGlZzY6ycI"
    )
  )
  ...
  val (baseUrl, context) = gitHubAccount match {
    case Some(account) =>
      (s"http://$account.github.io/lagom.github.io", "/lagom.github.io")
  ...
  val blogPostsByTag =
    blogPostTags.map(tag => tag -> blogPosts.filter(_.tags.contains(tag)))
  val blogSummary = {
  ...
  implicit val lagomContext = LagomContext(
    baseUrl,
    context,
    currentLagomVersion,
    currentDocsVersion,
  ...
  def generatePage(name: String,
                   template: Template1[LagomContext, Html]): OutputFile = {
  ...
  def savePage(name: String,
    ...
    val sitemapUrl = name match {
      case "index.html" => ""
      case index if index.endsWith("/index.html") =>
        index.stripSuffix("/index.html")
      case other => other
    }
  ...
  def renderMarkdownFiles(path: String, file: File): Seq[OutputFile] = {
    if (file.isDirectory) {
      ...
      file.listFiles().flatMap { child =>
        val childPath =
          if (path.isEmpty) child.getName else path + "/" + child.getName
      ...
      lines.dropWhile(!_.startsWith("#")) match {
        ...
        case Nil =>
          throw new IllegalArgumentException(
            "Markdown files must start with a heading using the # syntax"
          )
      }
    }
  }
  ...
  val blogPostFiles = {
    ...
    renderedBlogPosts.map {
      case (post, renderedPost) =>
        val fixedLinks =
          if (context.nonEmpty)
            FeedFormatter.makeAbsoluteLinks(renderedPost, context)
          else renderedPost
      ...
      case (tag, posts) =>
        ...
        savePage(
          s"blog/tags/$tag.html",
          html.blog(
            s"Blog posts tagged with $tag",
            renderRecent = true,
            postSummaries
          )
        )
      ...
      savePage(
        "blog/index.html",
        html.blog("Blog", renderRecent = false, blogPostSummaries.take(10)),
        sitemapPriority = "0.5"
      )
    } :+ {
      // Feed
      ...
      XML.save(
        ...
        FeedFormatter.atom(renderedBlogPosts.take(10)),
        enc = "utf-8"
      )
  ...
  // Discover versions
  val versions = docsDir
    .listFiles()
    .toSeq
    .map { versionDir =>
      // Discover languages
      val languages = versionDir
        .listFiles()
        .toSeq
        .map { languageDir =>
  ...
  private def getNav(ctx: Context, acc: List[Section] = Nil): List[Section] = {
    ctx.parent match {
      case None => acc
      case Some(parent) =>
        val titles = parent.children map {
          case (t, u) => NavLink(t, u, t == ctx.title)
        }
  ...
  def renderDocVersion(version: LanguageVersion): Seq[OutputFile] = {
    ...
    val languageVersions = versions.collect(
      Function.unlift((v: Version) => v.versionFor(version.language))
    )
    val currentLanguageVersion =
      currentVersion.flatMap(_.versionFor(version.language))
    ...
    def getNext(ctx: Context): Seq[NavLink] = {
      ctx.nextUrls match {
        ...
        case Some(urls) =>
          urls.flatMap { url =>
            version.toc.mappings
              .get(url)
              .map(ctx => NavLink(ctx.title, url, false))
              .toSeq
          }
      }
    }
      ...
      if (file.isDirectory) {
        ...
        val rendered = version.toc.mappings.get(path) match {
          case Some(context) if !context.nostyle =>
            ...
            val canonical =
              currentLanguageVersion.map(_.pageFor(path)).collect {
                case VersionPage(name, true, _) =>
                  s"$baseUrl/documentation/$name/${version.language}/$path"
              }

            val nextLinks = getNext(context)

            val rendered = html.documentation(
              path,
              fileContent,
              context,
              version,
  ...
  val generated = templatePages.map((generatePage _).tupled) ++
    ...
    versions.map { version =>
      savePage(
        s"documentation/${version.name}/index.html",
        html.documentationVersionIndex(version),
        includeInSitemap = false
      )
    } ++
  ...
  // sitemaps
  val mainSitemap = Sitemap(
    "sitemap-main.xml",
    generated
      .filter(_.includeInSitemap)
      .map(file => SitemapUrl(file.sitemapUrl, file.sitemapPriority))
  )

  val docsSitemap = Sitemap(
    "sitemap-docs.xml",
    generatedDocs
      .find(_._1.name == currentDocsVersion)
      .map {
        case (_, pages) =>
          pages.collect {
            case OutputFile(_, path, true, sitemapPriority) =>
              SitemapUrl(path, sitemapPriority)
          }
      }
      .getOrElse(Nil)
  )

  val generatedSitemaps =
    Sitemap.generateSitemaps(outputDir, baseUrl, Seq(mainSitemap, docsSitemap))

  val generatedSet: Set[File] = generated
    .map(_.file)
    .toSet ++ generatedSitemaps ++ generatedDocs.flatMap(_._2.map(_.file))
...
case class OutputFile(file: File,
                      sitemapUrl: String,
...
/**
  ...
  */
case class LagomContext(baseUrl: String,
                        path: String,
                        currentLagomVersion: String,
...
case class CommunityContent(description: String,
                            href: String,
                            hrefTitle: String)
...
case class Version(name: String, languages: Seq[LanguageVersion]) {
  def versionFor(language: String): Option[LanguageVersion] =
    languages.find(_.language == language)
}
...
/**
  ...
  */
case class LanguageVersion(name: String,
                           language: String,
                           file: File,
                           toc: TOC) {
  def pageFor(path: String) =
    VersionPage(name, toc.mappings.get(path).isDefined, path)
  ...
  private val apiRoot = (name, language) match {
    ...
    case (version, "scala") if version >= "1.4" =>
      "api/com/lightbend/lagom/scaladsl/index.html"
    case (version, "scala") if version < "1.4" =>
      "api/index.html#com.lightbend.lagom.scaladsl.package"
  }

  def referenceManualPath(implicit ctx: LagomContext) =
    s"${ctx.path}/documentation/$name/$language/Home.html"
  def apiDocPath(implicit ctx: LagomContext) =
    s"${ctx.path}/documentation/$name/$language/$apiRoot"
}
